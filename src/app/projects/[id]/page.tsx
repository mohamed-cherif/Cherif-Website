...
import { getProject, orderedProjects, type ProjectDemo } from "@/data/projects";
...
export default async function ProjectPage({ params }: Props) {
  ...
  // Lead with the demo GIF when there is one, otherwise the first photo.
  const demos = project.demos ?? [];
  const leadDemo = demos[0];
  const leadImage = leadDemo ? undefined : project.images[0];
  const galleryImages = leadImage ? project.images.slice(1) : project.images;
  const galleryDemos = demos.slice(1);
  const hasLead = Boolean(leadDemo || leadImage);
    ...
    <>
      ...
      <main className="pt-28 pb-20" style={{ background: "var(--color-bg)" }}>
        <article className="max-w-5xl mx-auto px-6">
          ...
          {/* Lead media + overview */}
          <div className={`mt-12 grid gap-10 items-start ${hasLead ? "md:grid-cols-2" : ""}`}>
            {hasLead && (
              <div className="md:sticky md:top-24">
                {leadDemo && <DemoFigure demo={leadDemo} preload />}
                {leadImage && (
                  <ImageFigure
                    src={leadImage}
                    alt={project.title}
                    caption={project.captions?.[leadImage]}
                    preload
                  />
                )}
              </div>
          ...
          {/* Gallery */}
          {(galleryImages.length > 0 || galleryDemos.length > 0) && (
            <section className="mt-14">
